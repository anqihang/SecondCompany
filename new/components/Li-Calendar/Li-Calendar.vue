<template>
	<view class="bg-white">
		<view class='calendar-Time-header under_line'>
			...
			<text class='calendar-backToToday text-lg' @tap="backToToday"> <text class="round cuIcon-focus">今</text>
			</text>
		</view>
		<view class="calendar-content">
			<view class="calendar-row calendar-header">
				<view class="calendar-col aligncanter" style="color:#d84e41;font-size:34rpx;">日</view>
				<view class="calendar-col aligncanter" style="font-size:34rpx;">一</view>
				<view class="calendar-col aligncanter" style="font-size:34rpx;">二</view>
				<view class="calendar-col aligncanter" style="font-size:34rpx;">三</view>
				<view class="calendar-col aligncanter" style="font-size:34rpx;">四</view>
				<view class="calendar-col aligncanter" style="font-size:34rpx;">五</view>
				<view class="calendar-col aligncanter" style="color:#d84e41;font-size:34rpx;">六</view>
			</view>
			<view class="calendar-row" v-bind:style="{'transform':transformObj, 'transition-duration':transformTimeObj}"
				...
				<view class="calendar-col aligncanter calendar-col-lastMonth" v-for='item in beforeDateList'
					...
					<view class='dayValue'>
						...
						<text class='calendar-point'
							v-bind:style="{'color':item.pointTextColor?item.pointTextColor:maskColor}">{{item.pointText}}</text>
						<text class='calendar-text'>{{item.markText}}</text>
					</view>
				</view>
				<view class="calendar-col aligncanter currentDays"
					v-bind:class="{'calendar-today' : item.isToday && item.dateIndex == currentSelectTime,'calendar-active' : !item.isToday && item.dateIndex == currentSelectTime }"
					:style="{'background-color':(!item.isToday && item.dateIndex == currentSelectTime)?'#d84e41':'','color':(!item.isToday && item.dateIndex == currentSelectTime)?'white':''}"
					v-for='item in dateList' v-bind:key='item.key' @tap="selectedDateFun(item.dateIndex)">
					<view class='dayValue' style="font-weight:500;font-size: 40rpx;">
						<text class='calendar-date'
							:class="{'calendar-day-6-7':(item.isDaySunOrSat && item.pointText != '班')}">{{item.dateIndex}}</text>
						<text class='calendar-point round' v-if="item.pointText"><text
								class="text-xs text-bold text-shadow"
								:class="{'text-blue':item.pointText == '班','text-red':item.pointText == '休'}">{{item.pointText}}</text></text>
						<text class='calendar-text' style="padding-top:18rpx;font-size:25rpx;"
							v-bind:style="{'color':item.markTextColor?item.markTextColor:maskColor,'color':item.dateIndex == currentSelectTime?'white':''}">{{item.markText}}</text>
					</view>
				</view>
	...
	export default {
		...
		data() {
			return {
				nextDisabled: false,
				lastDisabled: false,
				currentSelectTime: (new Date().getDate()), //点击时保存的dateIndex
				title_time: '', //顶头文本
				year: undefined, //当前年
				month: undefined, //当前月
		...
		methods: {
			render(setTimeStr) { //初始化
				...
				for (let i = 0, j = weeks.length; i < j; i++) {
					...
					for (let k = 0, l = days.length; k < l; k++) {
						...
						if (!tempObj.markText) {
							// 农历
							let tempLunarDay = lu.getDayInChinese() == '初一' ? lu.getMonthInChinese() + '月' : lu
								.getDayInChinese();
							tempObj.markText = tempLunarDay;
							tempObj.markTextColor = '#8799a3';
						}
						// 当日
						if (days[k].getYear() == t.getYear() && days[k].getMonth() == t.getMonth() && days[k].getDay() == t
							.getDay()) {
							tempObj.isToday = true;
						}
						if (m.getYear() > days[k].getYear() || (m.getYear() == days[k].getYear() && m.getMonth() > days[k]
								.getMonth())) {
						...
						} else if (m.getYear() < days[k].getYear() || (m.getYear() == days[k].getYear() && m.getMonth() <
								days[k].getMonth())) {
			...
			dateChange(e) {
				...
				//如果已经是当月
				if (selectDate.getFullYear() == this.year && (selectDate.getMonth() + 1) == this.month && selectDate
					.getDate() == t.currentSelectTime) {
